---
title: "06 - Usage inside a monorepo"
...
2. Download and extract the docs-template into your monorepo
   ...
   (Add `docs` to the `packages` array if it isn't already present.)
...
4. Check and resolve FIXME comments inside workflows
   ...
   - Open the workflow files and search for `FIXME` markers. They typically indicate environment-specific values, secrets, or configuration you must provide (e.g., Fly app name, secret names, or publishing settings). Resolve those before relying on the workflows.
...
- If `pnpm run dev` doesn't start, check that you executed it from the repo root and that `docs/package.json` contains the `dev` script (template normally wires this up).
- If the site shows example content, remove the template example files from `docs/content` and add your own pages.

That's it — these steps will get the template running inside your monorepo and prepared for CI/publishing. If you want, I can also:

- Verify `pnpm-workspace.yaml` in your repo includes `docs` (I can open the file and confirm if you want).
- Run a quick smoke test locally (you'll need to allow me to run commands).

If you'd like me to make further adjustments (different wording, screenshots, or additional examples), tell me what to include.
